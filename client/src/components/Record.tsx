...
function Record() {
  ...
  const downoald = () => {
    ...
    if (fileType === 'json') {
      output = JSON.stringify(
        {
          record: pathname === '/students' ? record : record,
        },
        null,
        4
      );
      ...
      output += `${'Num'.padEnd(10, ' ')} ${'Fname'.padEnd(
        ...
        30,
        ' '
      )} ${'SessNum'.padEnd(10, ' ')} ${'Sub1'.padEnd(10, ' ')} ${'Sub2'.padEnd(
        10,
        ' '
      )} ${'Sub3'.padEnd(10, ' ')}\n`;
      ...
      record.forEach((s) => {
        output += `${`${s.NUM}`.padEnd(10, ' ')} ${`${s.FNAME}`.padEnd(
          ...
          30,
          ' '
        )} ${`${s.SESSNUM}`.padEnd(10, ' ')} ${`${s.SUB1}`.padEnd(
          10,
          ' '
        )} ${`${s.SUB2}`.padEnd(10, ' ')} ${`${s.SUB3}`.padEnd(10, ' ')}\n`;
      });
    } else {
      let contents = [];
      contents.push([
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <select
        className="cursor-pointer  mx-2 bg-white-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 p-2 px-3 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
        value={fileType}
