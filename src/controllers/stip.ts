...
export const getStips = async (req: Request, res: Response) => {
  ...
  const stips = await dbConnection.execute(
    'SELECT * FROM STIP ORDER BY STIP.Num ASC'
  );
...
export const getStip = async (req: Request, res: Response) => {
  ...
  const stip = await dbConnection.execute(
    'SELECT * FROM STIP WHERE STIP.Num = :1',
    [req.params.id]
  );
...
export const deleteStip = async (req: Request, res: Response) => {
  ...
  await dbConnection.execute('DELETE FROM STIP WHERE STIP.Num = :1', [
    req.params.id,
  ]);
...
export const updateStip = async (req: Request, res: Response) => {
  ...
  await dbConnection.execute(
    'UPDATE STIP SET STIP.Salary = :1 WHERE STIP.Num = :2',
...
export const createStip = async (req: Request, res: Response) => {
  ...
  const { NUM, SALARY } = req.body;

  await dbConnection.execute(
    'INSERT INTO PAS_6308.STIP (Num, Salary) VALUES (:1, :2)',
    [NUM, SALARY]
  );
